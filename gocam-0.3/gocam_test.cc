...
analyse_step_by_step()
{
  ...
  //newCImgDisplay(analyser.line_image, "The line image");
  //newCImgDisplay(analyser.weighted_line_image, "The weighted line image");

  analyser.compute_hough_image();
  //newCImgDisplay(analyser.hough_image, "The hough image");
...
//   draw_grid(tmp, white);
  //newCImgDisplay(tmp, "The initial grid lines");
	 ...
	 (int)analyser.lines[1].size() < analyser.board_size)
  {
    ...
    analyser.tune_grid();
     ...
     draw_grid(tmp, white);
    //newCImgDisplay(tmp, "Growing the grid");
  }
}
